  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="relative overflow-hidden">
        <div className="absolute inset-0 bg-cover bg-center" style={{ backgroundImage: 'url(https://images.unsplash.com/photo-1474487548417-781cb71495f3?auto=format&fit=crop&w=1600&q=80)', filter: 'saturate(1.8) brightness(0.8) contrast(1.15)' }} />
        <div className="absolute inset-0 bg-blue-950/72" />
        <div className="relative z-10 py-10 px-4">
          <div className="max-w-6xl mx-auto">
            <div className="mb-6 text-white">
              <h1 className="text-3xl font-bold flex items-center gap-2 mb-1"><Train className="w-7 h-7" /> Train Tickets</h1>
              <p className="text-blue-200 text-sm">Fast &amp; scenic rail journeys across India</p>
            </div>
              ...
              <div className="rounded-2xl bg-white shadow-xl">
                <div className="flex flex-col divide-y divide-gray-100 lg:flex-row lg:items-stretch lg:divide-x lg:divide-y-0">
                  ...
                  <div className="px-4 py-4 lg:w-[200px]">
                    <DatePickerInput label="DATE" value={date} min={today} onChange={setDate} accentColor="blue" />
                  </div>
                  <div className="px-4 py-4 lg:w-[170px]">
                    <label className="block text-[10px] font-semibold uppercase tracking-widest text-gray-400 mb-2">CLASS</label>
                    <select value={trainClass} onChange={e => setTrainClass(e.target.value)}
                      className="w-full border-b border-gray-200 py-1 text-sm font-medium text-gray-800 focus:outline-none focus:border-blue-500 bg-transparent">
                      {CLASSES.map(c => <option key={c.code} value={c.code}>{c.label}</option>)}
                    </select>
                  </div>
                  <div className="px-4 py-4 lg:w-[120px]">
                    <label className="block text-[10px] font-semibold uppercase tracking-widest text-gray-400 mb-2">PASSENGERS</label>
                    <input type="number" value={passengers} min={1} max={6}
                      onChange={e => setPassengers(Number(e.target.value))}
                      className="w-full border-b border-gray-200 py-1 text-sm font-medium text-gray-800 focus:outline-none focus:border-blue-500" />
                  </div>
                  <div className="px-4 py-4 lg:flex lg:items-center lg:justify-center">
                    <button type="submit" className="flex w-full items-center justify-center gap-2 rounded-full px-10 py-3 font-bold text-white shadow-lg transition-all hover:shadow-xl hover:scale-105 active:scale-100 lg:w-auto" style={{ background: 'linear-gradient(90deg, #1e40af, #818cf8)' }}>
