...
import { ArrowLeft, Bus, Clock, MapPin, Users, Tag, CreditCard, Shield, X } from 'lucide-react'
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <Link to="/buses" className="text-orange-500 hover:underline">Search Buses</Link>
        </div>
      </div>
    )
  }

  const basePrice  = bus.price * seats
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-orange-500 text-white py-4 px-4">
        <div className="max-w-5xl mx-auto flex items-center gap-3">
          <button onClick={() => navigate(-1)} className="p-1 rounded hover:bg-orange-400 transition-colors">
            <ArrowLeft size={20} />
          </button>
          <Bus size={22} />
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-sm text-gray-500">{bus.busType}</span>
            </div>
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xl font-bold text-gray-900">{fmt(bus.departureTime)}</p>
                <p className="text-sm text-gray-600 flex items-center gap-1"><MapPin size={12} />{bus.origin}</p>
              </div>
              ...
              <div className="text-right">
                ...
                <p className="text-sm text-gray-600 flex items-center gap-1 justify-end"><MapPin size={12} />{bus.destination}</p>
              </div>
            </div>
            <div className="mt-3 pt-3 border-t border-gray-100 flex items-center gap-4 text-sm text-gray-500">
              <span className="flex items-center gap-1"><Users size={14} />{seats} passenger{seats > 1 ? 's' : ''}</span>
              {bus.acAvailable && <span className="bg-blue-50 text-blue-600 px-2 py-0.5 rounded text-xs font-medium">AC</span>}
              {bus.isRefundable && <span className="bg-green-50 text-green-600 px-2 py-0.5 rounded text-xs font-medium">Refundable</span>}
            </div>
          </div>
            ...
            <form id="book-form" onSubmit={handleSubmit(onSubmit)}>
              <div className="grid grid-cols-2 gap-4 mb-4">
                <div>
                  ...
                  <input {...register('firstName')} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400" placeholder="First name" />
                  {errors.firstName && <p className="text-red-500 text-xs mt-1">{errors.firstName.message}</p>}
                </div>
                <div>
                  <label className="block text-sm text-gray-600 mb-1">Last Name *</label>
                  <input {...register('lastName')} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400" placeholder="Last name" />
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input {...register('email')} type="email" className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400" placeholder="email@example.com" />
                  {errors.email && <p className="text-red-500 text-xs mt-1">{errors.email.message}</p>}
                </div>
                <div>
                  <label className="block text-sm text-gray-600 mb-1">Phone *</label>
                  <input {...register('phone')} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400" placeholder="+91 9999999999" />
            ...
            {couponApplied ? (
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 border border-gray-300 rounded-lg px-3 py-2 text-sm uppercase focus:outline-none focus:ring-2 focus:ring-orange-400"
                />
                <button
                  type="button"
                  onClick={applyCoupon}
          ...
          {isLoggedIn && (
            <div className="bg-white rounded-xl border border-gray-200 p-4">
              <div className="flex items-center justify-between mb-3">
                ...
                <button type="button" onClick={() => setShowAddCard(true)} className="text-xs text-orange-500 font-semibold hover:underline">+ Add Card</button>
              </div>
        ...
        <div className="space-y-4">
          <div className="bg-white rounded-xl border border-gray-200 p-4 sticky top-4">
            ...
            <button
              ...
              type="submit"
              disabled={submitting}
              className="mt-4 w-full bg-orange-500 hover:bg-orange-600 text-white font-bold py-3 px-4 rounded-xl transition-colors disabled:opacity-50"
            >
            ...
            {!isLoggedIn && (
              <p className="mt-2 text-center text-xs text-gray-500">
                <Link to="/login" className="text-orange-500 font-semibold">Login</Link> to complete booking
              </p>
            )}
